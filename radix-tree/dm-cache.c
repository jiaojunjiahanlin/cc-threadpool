...
static void getblock(struct list_head * lru, int index , struct flush * in );
...
struct workqueue_struct *_kcached_wq;
...
static int do_fetch(struct kcached_job *job)
{
	...
	if (bio_data_dir(bio) == READ) { /* The original request is a READ */
		...
		if (head && tail) { /* Special case */
			bvec = kmalloc(job->nr_pages * sizeof(*bvec), GFP_NOIO);
			if (!bvec) {
				DMERR("do_fetch: No memory");
				return 1;
			}
...
static void write_back(struct cache_c *dmc,sector_t index, unsigned int length)
{
	...
	getblock(dmc->lru,index, &x);
	cacheblock = x.realblock;
	printk("Write back block %llu(%llu, %u)\n",
	...
	for (i=0; i<length; i++)
	{	
		//set_state(getblock(dmc->lru,index+i,&x).state, WRITEBACK);
		writecache = radix_tree_lookup(dmc->cache, index+i);
		if(writecache != NULL){
...
static void cache_flush(struct cache_c *dmc)
{
	...
	printk("Flush dirty blocks (%llu) ...", (unsigned long long) dmc->dirty_blocks);
	while (i< dmc->size) {
		j = 1;
		struct flush x;
		getblock(dmc->lru,i,&x);
		
		if (is_state(x.state, DIRTY) && !is_state(x.state, WRITEBACK) && !is_state(x.state, RESERVED)) {
			//struct flush y;
			//getblock(dmc->lru,i+j,&y);
			/*while ((i+j) < dmc->size && is_state(y.state, DIRTY) && !is_state(y.state, WRITEBACK) 
			&& !is_state(y.state, RESERVED) && (y.block == x.block + j * dmc->block_size)) {
				j++;
				getblock(dmc->lru,i+j,&y);
			}*/
			dmc->dirty += j;
		//	printk("dirty blocks form %llu to %d\n",i, j);	
			write_back(dmc, i, j);
		}
		i += j;
	}
	...
	while (i< dmc->size) {
		j = 1;
		if (is_state(cache[i].state, DIRTY) && !is_state(cache[i].state, WRITEBACK) && !is_state(cache[i].state, RESERVED)) {
			while ((i+j) < dmc->size && is_state(cache[i+j].state, DIRTY && !is_state(cache[i+j].state, WRITEBACK) 
				&& !is_state(cache[i+j].state, RESERVED)) && (cache[i+j].block == cache[i].block + j *
			        dmc->block_size)) {
				j++;
			}
			dmc->dirty += j;
...
{
	sector_t i;
	struct listhead *list = dmc->lru;
	struct listhead *temp;
...
int LRU(struct cache_c *dmc, sector_t block,
...
{
	int res;
	res = dmc->caching_algorithm(dmc, block, cache_block);
	return res;
}
...
{
	//struct radix_tree_root temp_cache = dmc->cache;
	//struct cacheblock *cache = radix_tree_lookup(&temp_cache, cache_block);
...
//					dmc->cache_dev->name, bio->bi_sector);
		...
		spin_unlock(&cache->lock);
		//getnstimeofday(&time);
//		if (unlikely(dmc->flushed== 0 && (dmc->dirty_blocks > dmc->size*.1 || time.tv_sec >= dmc->last)))
//		{
//			dmc->flushed=1;
//			queue_work(_kcached_wq, &dmc->active_flush);//cache_flush(dmc);
//		}
		return 1;
	}
}
...
static int cache_miss(struct cache_c *dmc, struct bio* bio) {
	if (bio_data_dir(bio) == READ)
		return cache_read_miss(dmc, bio);
	else
		return cache_write_miss(dmc, bio);
}
		      ...
		      union map_info *map_context)
{
	...
	if (unlikely(dmc->flushed == 0 && dmc->dirty_blocks > dmc->size*.1))
	{
		dmc->flushed = 1;
		flush(dmc);
	}
	...
	DMINFO("Got a %s for %llu ((%llu:%llu), %u bytes)",
...
static void getblock(struct list_head * lru, int index , struct flush * in ){
	...
	sector_t result;
	list_for_each(temp, lru)
        {
                in->block = list_entry(temp, struct block_list, list)->block->block;
		in->realblock = list_entry(temp, struct block_list, list)->block;
		in->state = list_entry(temp, struct block_list, list)->block->state;
		if(i == index)
		{
			break;
	        }
...
static unsigned short getstate(struct list_head * lru, int index){
        int i = 0;
        struct list_head *temp;
        unsigned short result;
        list_for_each(temp, lru)
        {
                result = list_entry(temp, struct block_list, list)->block->state;
                if(i == index)
                {
                        break;
...
/*	list_for_each(temp, dmc->lru)
	{
		...
		block = list_entry(temp, struct block_list, list)->block; 
		//meta_data[j] = block->state ? block->block : 0;
		printk("here is the block %d\n", block->block);
		j++;
        }
	printk("here is j %d\n", j);
*/
	...
	while(index < meta_size) {
		...
		for (i=to_bytes(index)/sizeof(sector_t), j=0;
			 ...
			 */
			getblock(dmc->lru,i,&x);	
			//printk("i %d j %d block %llu state %hu\n",i,j,x.block,x.state);
			meta_data[j] = x.state ? x.block : 0;
		}
		chksum = csum_partial((char *)meta_data, to_bytes(where.count), chksum);
	...
	DMINFO("Store metadata to disk: block size(%u), cache size(%llu), " \
...
/*This funtion was added by Doug Otstott and Stephen Bromfield to faciliate active flushing during writeback mode
**June 2011 */

static void active_flush(struct work_struct *work)
{
	struct cache_c * dmc;

	dmc = container_of(work, struct cache_c, active_flush);
	cache_flush(dmc);
	if(dmc->shouldEnd == 0)
		mod_timer(&dmc->flush_time, jiffies + msecs_to_jiffies(30000));
}

void flush(struct cache_c * dmc)
{
	queue_work(_kcached_wq, &dmc->active_flush);
}
...
init:
	...
	blocks = (struct cacheblock *) vmalloc(dmc->size * (sizeof(struct cacheblock)));
		...
		temp[i].block = &blocks[i];
                ...
                temp[i].block->cache=i;
                list_add(&temp[i].list, dmc->lru);
        }

//	list_for_each(ptr, dmc->lru)
//	{
//		DMINFO("Block: %llu", list_entry(ptr, struct block_list, list)->block->block); 
//	}
...
static void cache_dtr(struct dm_target *ti)
{
	...
	struct block_list *tmp, *item;

	dmc->shouldEnd = 1;
	del_timer(&dmc->flush_time);
	cache_flush(dmc);



	//flush_workqueue(_kcached_wq);
	//lock_map_acquire(&_kcached_wq->lockdep_map);
        //cache_flush(dmc);
	//lock_map_release(&_kcached_wq->lockdep_map);
	//flush_bios(dmc->cache);
	...
	list_for_each_entry_safe(item,tmp,dmc->lru, list)
	{
		//vfree(item->block);
		//vfree(item);	
	}
