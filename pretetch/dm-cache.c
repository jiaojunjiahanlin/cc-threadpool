...
static unsigned long pf_hash_block(struct cache_c *dmc, sector_t block,unsigned int block_shift);
static int pf_cache_hit(struct cache_c *dmc, struct bio* bio, sector_t cache_block,unsigned int block_mask,unsigned int block_shift);
static int pf_cache_lookup(struct cache_c *dmc, sector_t block,
	                    sector_t *cache_block,unsigned int block_shift,unsigned int block_size);
static int pf_cache_read_miss(struct cache_c *dmc, struct bio* bio,
	                       sector_t cache_block,unsigned int block_mask,unsigned int block_size);
...
static unsigned long pf_hash_block(struct cache_c *dmc, sector_t block,unsigned int block_shift)
{
	...
	value = (unsigned long)(block >> (block_shift +
	        dmc->consecutive_shift));
	set_number = hash_long(value, dmc->bits) / dmc->assoc;
...
static int pf_cache_lookup(struct cache_c *dmc, sector_t block,
	                    sector_t *cache_block,unsigned int block_shift,unsigned int block_size)
{
	unsigned long set_number = pf_hash_block(dmc, block,block_shift);
	sector_t index;
	int i, res;
	unsigned int cache_assoc = dmc->assoc;
	struct cacheblock *cache = dmc->cache;
...
static int pf_cache_hit(struct cache_c *dmc, struct bio* bio, sector_t cache_block,unsigned int block_mask,unsigned int block_shift)
{
	unsigned int offset = (unsigned int)(bio->bi_sector & block_mask);
	struct cacheblock *cache = dmc->cache;
...
static int pf_cache_read_miss(struct cache_c *dmc, struct bio* bio,
	...
	if (left < block_size) {
		...
		tail = to_bytes(block_size) - bio->bi_size - head;
